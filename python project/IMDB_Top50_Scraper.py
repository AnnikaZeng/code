...
if input_year > current_year:
    ...
    for year in tqdm(range(input_year, current_year + 1)):
        sys.stdout = open(
            os.path.join(dataset_location, "IMDB_Top50_" + str(year) + ".txt"), "w+"
        )
        url = (
            "http://www.imdb.com/search/title?release_date="
            + str(year)
            + ","
            + str(year)
            + "&title_type=feature"
        ...
        # === PATCH BEGINS ===
        header = soup.find("h1")
        print((header.get_text(strip=True) + ":") if header else f"{year}:")
        # === PATCH ENDS ===
