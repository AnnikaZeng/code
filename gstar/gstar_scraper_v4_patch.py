...
# -*- coding: utf-8 -*-
"""
G-STAR RAW scraper (Playwright) – v4 patch

This patch only changes two things on top of your v4:
1) **CSV排序**：不再对CSV做二次排序，直接保留“按爬取顺序”的行序（即分区→品类→页面顺序）。
2) **Kids链接**：侧栏在 Kids 下的链接是 `/en_us/shop/boys` 与 `/en_us/shop/girls`，
   以前仅匹配 `/shop/kids/...` 会漏抓；已在侧栏解析中单独放行 boys/girls。
...
class GStarScraper:
    ...
    def _get_leaf_links(self, page: Page, section: str) -> List[Tuple[str, str]]:
        ...
        for label in wanted:
            # 找到侧栏分支标题（button 或 a），精确匹配文本
            hdr = sidebar.locator(
                f"xpath=//*[self::button or self::a or @role='button'][normalize-space()='{label}']"
            ).first
            ...
            container = sidebar.locator(
                f"xpath=(//*[self::button or self::a or @role='button'][normalize-space()='{label}']/following-sibling::*[1])"
            ).first
            if container and container.count():
                self._expand_all_toggles(container)
                # ★ kids 特殊：boys/girls 不在 /shop/kids/ 路径下
                if section == "kids":
            ...
            for a in anchors:
                ...
                if "shop all" in text.lower():
                    continue
                # kids 允许 /shop/boys|/shop/girls；其他章节需包含 /shop/{section}/
